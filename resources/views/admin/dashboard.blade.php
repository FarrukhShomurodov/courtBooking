...
@section('content')
    ...
    @if (auth()->check() && auth()->user()->hasRole('owner stadium') && !$isOwner)
        <div class="d-flex justify-content-center align-items-center vh-100">
            <form id="formAuthentication" class="mb-3 card p-3" action="{{ route('owner.confirmation') }}"
                  method="POST">
                @csrf
                <input type="hidden" class="form-control" id="login" name="login"
                       placeholder="{{ __('login.Введите ваш логи') }}" autofocus value="{{auth()->user()->login}}">
