...
@section('title')
    <title>{{'Frest - '. __('stadium.edit_stadium') }}</title>
@endsection

@section('content')
    <h6 class="py-3 breadcrumb-wrapper mb-4">
        <span class="text-muted fw-light"><a class="text-muted"
                                             href="{{route('stadiums.index')}}">{{  __('menu.Стадион') }}</a> /</span>{{ __('stadium.edit_stadium') }}
    </h6>
    <div class="card mb-4">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">{{ __('stadium.edit_stadium') }}</h5>
            ...
            <form id="stadiumForm" action="{{ route('stadiums.update', $stadium->id) }}" method="POST"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" class="form-control" id="stadium-name"
                           placeholder="{{ __('stadium.name') }}" value="{{ $stadium->name }}"
                           required>
                </div>
                <div class="mb-3">
                ...
                <button type="submit" class="btn btn-warning">{{ __('stadium.edit') }}</button>
            </form>
        </div>
    </div>
@endsection
